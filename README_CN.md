...
(1)实现 TCP 通信：服务器网络相交于客户端有所不同的地方在于，建立TCP 通信时，要创建一个专门的套接字绑定固定的端口和 IP 用于监听户端的连接请求。

(2) 处理信息高效：保证服务器能同时处理接收不同客户端发来的消息，即采取多线程的高效处理方式，保证客户端能够及时接收到消息。

(3) 保证消息的接收和发送同步：保证消息的先后顺序，即先收到的消息先发送。
